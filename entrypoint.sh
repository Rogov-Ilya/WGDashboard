...
ensure_installation() {
  ...
  if [ -z "$(ls -A "${WGDASH}")" ]; then
    ...
    mv  /usr/lib/python3/psutil* "${WGDASH}"/src/venv/lib/python3.10/site-packages
    mv  /usr/lib/python3/bcrypt* "${WGDASH}"/src/venv/lib/python3.10/site-packages
  ...
  if [ ! -f "/etc/amnezia/amneziawg/awg0.conf" ]; then
    echo "Standard awg0 Configuration file not found, grabbing template."
    cp -a "/setup/conf/awg0.conf" "/etc/amnezia/amneziawg/awg0.conf"
    ...
    sed -i "s|^PrivateKey =$|PrivateKey = ${privateKey}|g" /etc/amnezia/amneziawg/awg0.conf
    sed -i "s|^PrivateKey *=.*$|PrivateKey = ${privateKey}|g" /etc/amnezia/amneziawg/awg0.conf
    echo "Done setting template."
  else
    echo "Existing awg0 configuration file found, using that."
  fi
}
